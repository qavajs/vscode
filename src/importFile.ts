...
export async function importFile(filePath: string) {
	if (vscode.workspace.workspaceFolders) {
		...
		switch (await getFileType(filePath)) {
			...
			case FileType.TS: return eval(`(() => { require('${normalize(getTSNodePath() as string)}').register({transpileOnly: true, project: '${normalize(cwd)}'}); return require('${normalize(filePath)}'); })()`);
